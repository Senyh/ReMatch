...
from wheels.colorspace_transform import rgb_to_ycbcr_batch, ycbcr_to_rgb_batch
...
def get_data(args):
    ...
    labeled_train_set = FundusDataset(image_path=args.data_path, stage='train', image_size=args.image_size, is_augmentation=True, labeled=True, percentage=args.labeled_percentage, modality=args.train_modality)
    unlabeled_train_set = FundusDataset(image_path=args.data_path, stage='train', image_size=args.image_size, is_augmentation=True, labeled=False, percentage=args.labeled_percentage, modality=args.train_modality)
...
def main(is_debug=False):
    ...
    denoise_model = networks.__dict__[args.backbone](in_channels=1, out_channels=1).to(device)
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        for idx in pbar:
            # sparse labeled data
            limage, label, limageA1, limageA2 = iter_train_labeled_dataloader.next()
            ...
            with torch.no_grad():
                ...
                Su = Stu * wu + Ssu * (1 - wu)
                uimageA2Y = Usu @ torch.diag_embed(Su) @ Vhsu
                uimageA2Y = torch.clamp(uimageA2Y, 0, 1)
                denoise_model.eval()
                uimageA2Y = denoise_model(uimageA2Y)['out']
                
                p5_uimageA2Y = torch.quantile(uimageA2Y, 0.01)
                p95_uimageA2Y = torch.quantile(uimageA2Y, 0.99)
                uimageA2Y = torch.clamp(uimageA2Y, p5_uimageA2Y, p95_uimageA2Y)
                uimageA2Y = instance_max_min_normalize(uimageA2Y)
                uimageA2YCbCr = rgb_to_ycbcr_batch(uimageA2)
                uimageA2Yo = uimageA2YCbCr[:, 0].unsqueeze(1)
                uimageA2Y[uimageA2Yo == 0] = 0
                uimageA2YCbCr[:, 0] = uimageA2Y[:, 0]
                uimageA2 = ycbcr_to_rgb_batch(uimageA2YCbCr).detach()
            denoise_model.train()    
            ...
            for i in range(limageA1.shape[0]):
            ...
            with torch.no_grad():
                ...
                Sl = Stl * wl + Ssl * (1 - wl)
                limageA2Y = Usl @ torch.diag_embed(Sl) @ Vhsl
                limageA2Y = torch.clamp(limageA2Y, 0, 1).detach()
                # get y component for denosing criterion
                limageA1YCbCr = rgb_to_ycbcr_batch(limageA1)
                limageA1Y = limageA1YCbCr[:, 0].unsqueeze(1)
                # limageA2 = ycbcr_to_rgb_batch(limageA2YCbCr)
            limageA2Y = denoise_model(limageA2Y)['out']
            limageA2Y = instance_max_min_normalize(limageA2Y)
            # the supervised loss
            loss_s = criterion(pred_lA1_logits, label.squeeze(1).long()) 
            loss_d = criterion_d(limageA2Y, limageA1Y.detach()) * 10.
